...
class SendNotifikasjonTest {
    ...
    fun opprettMelding() {
        ...
        assertEquals(
            true,
            isWithin10Seconds(beskjed.getSynligFremTil().toLocalDateTime(), LocalDateTime.now().plusDays(7))
        )
...
fun isWithin10Seconds(actual: LocalDateTime, expected: LocalDateTime = LocalDateTime.now()): Boolean =
